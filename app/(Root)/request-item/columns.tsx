...
import AddNumberOfItems from "./AddNumberOfItems";
...
export const columns: ColumnDef<Doc<"purchaseItem">>[] = [
  ...
  // },
  {
    ...
    cell:({row})=>{
      ...
      return <p className=" text-start">{quantity.toLocaleString()} <span
      className={`capitalize text-xs ${status === 'in-stock' ? "text-green-600" : "text-orange-600"}`}
    >
  ...
  
  {
    ...
    cell:({row})=>{
      ...
      return <p className="uppercase">{unity}</p>
    }
  },
  
  {
    accessorKey: "_id",
    header: ()=>{
      return <p className="text-center">Number of items</p>

    },
    cell: ({ row }) => {
      const quantity = row.getValue("quantity") as number;
      const id = row.getValue("_id") as Id<"purchaseItem">;

      return <AddNumberOfItems quantity={quantity} id={id}/>;
    },
  },
  {
    header: "Actions",
